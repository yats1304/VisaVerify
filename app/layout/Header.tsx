...
const NOTIFICATIONS = [
  ...
  {
    id: 3,
    title: "Welcome to TheVisaWizard!",
...
export default function Header({ onMenuClick }: HeaderProps) {
  ...
  const title = PAGE_TITLES[pathname] ?? "TheVisaWizard";
      ...
      <div className="flex-1 flex items-center justify-between px-4 sm:px-6 border-b border-gray-100/60 dark:border-slate-800/60">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2.5">
            <div
              className="hidden sm:block w-[3px] h-5 rounded-full"
              style={{
                background: "linear-gradient(180deg, #7c3aed, #2563eb)",
              }}
            />
            <h1 className="text-base sm:text-[15px] font-bold text-gray-800 dark:text-slate-100 tracking-tight">
              {title}
            </h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  {notifications.map((n) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-sm leading-snug ${!n.read ? "font-semibold text-gray-900 dark:text-slate-50" : "font-medium text-gray-600 dark:text-slate-400"}`}
                        >
                          {n.title}
                        </p>
                        <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5 leading-relaxed">
                          {n.desc}
                        </p>
                        <p className="text-[11px] text-gray-300 dark:text-slate-600 mt-1 font-medium">
                          {n.time}
                        </p>
                      </div>
                      {!n.read && (
          ...
          <div
            className="w-px h-5 mx-0.5 sm:mx-1 rounded-full"
            style={{
              background:
                "linear-gradient(180deg, transparent, #a78bfa55, transparent)",
            }}
          />
          ...
          <div ref={profileRef} className="relative">
            ...
            >
              ...
              <span className="hidden sm:block text-sm font-semibold text-gray-800 dark:text-slate-100">
                TheVisaWizard
              </span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
