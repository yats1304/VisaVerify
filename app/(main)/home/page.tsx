...
function CrossIcon() {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
      fill="currentColor"
      className="w-4 h-4 shrink-0 text-gray-300 dark:text-slate-600"
    >
      <path
        fillRule="evenodd"
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm p-6 sm:p-8">
        ...
        <div className="flex flex-col gap-2 min-w-0">
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-slate-400 leading-relaxed max-w-2xl">
            TheVisaWizard is an AI-powered Visa Evaluations platform that
            validates visa and immigration documents instantly. Built for
            individuals, consultants, and enterprises - get accurate results
            without the manual overhead.
          </p>
          <div className="flex flex-wrap gap-2 mt-1">
            {["AI-Powered", "GDPR Compliant", "API Ready", "99.9% Uptime"].map(
              (tag) => (
                <span
                  key={tag}
                  className="text-xs font-medium px-2.5 py-1 rounded-lg bg-gray-50 dark:bg-slate-800 text-gray-500 dark:text-slate-400 border border-gray-100 dark:border-slate-700"
                >
                  {tag}
                </span>
              ),
      ...
      <div>
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">
            Everything you need to verify documents with confidence.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {features.map((f) => (
            <div
              key={f.title}
              className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm p-5 sm:p-6 flex flex-col gap-3"
            >
              <div
              ...
              <div>
                ...
                <p className="mt-1 text-xs sm:text-sm text-gray-500 dark:text-slate-400 leading-relaxed">
                  {f.description}
                </p>
              </div>
            </div>
      ...
      <div>
        <div className="mb-6 sm:mb-8 text-center">
          ...
          <p className="mt-1.5 text-sm text-gray-500 dark:text-slate-400">
            No hidden fees. Cancel any time. Start free — upgrade when
            you&apos;re ready.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 items-start">
          {plans.map((plan) => (
            ...
            >
              ...
              <div className="mb-4">
                <p
                  className={`text-sm font-bold ${plan.highlighted ? "text-[#7c3aed]" : "text-gray-500 dark:text-slate-400"}`}
                >
                  {plan.name}
                </p>
                <div className="flex items-end gap-1 mt-1.5">
                  <span className="text-3xl sm:text-4xl font-extrabold text-gray-900 dark:text-slate-50">
                    {plan.price}
                  </span>
                ...
                <p className="mt-2 text-xs sm:text-sm text-gray-500 dark:text-slate-400 leading-relaxed">
                  {plan.description}
                </p>
              </div>
              ...
              <ul className="flex flex-col gap-2.5">
                {plan.features.map((feat) => (
                  <li key={feat} className="flex items-start gap-2.5">
                    <CheckIcon
                      color={plan.highlighted ? "#7c3aed" : "#2563eb"}
                    />
                    <span className="text-xs sm:text-sm text-gray-600 dark:text-slate-300 leading-snug">
                      {feat}
                    </span>
                  </li>
                ))}
                {plan.missing.map((feat) => (
                  ...
                  >
                    <CrossIcon />
                    <span className="text-xs sm:text-sm text-gray-400 dark:text-slate-500 leading-snug line-through">
                      {feat}
                    </span>
                  </li>
                ))}
